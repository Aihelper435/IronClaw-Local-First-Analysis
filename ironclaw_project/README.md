...
<p align="center">
  <a href="#local-first-quick-start">Local-First Setup</a> •
...
**Run IronClaw entirely locally with no cloud account required!**
...
# 4. Configure for local operation
mkdir -p ~/.ironclaw
...
# 5. Run IronClaw (skip cloud onboarding)
...
```bash
./local-first-setup.sh
```

<details>
<summary><strong>🔧 Other Local Backend Options</strong></summary>

| Backend | Setup | Best For |
|---------|-------|----------|
| **Ollama** | `./local-first-setup.sh --backend=ollama` | Easy setup, great for most users |
| **LM Studio** | `./local-first-setup.sh --backend=lmstudio` | GUI interface, model browser |
| **vLLM** | `./local-first-setup.sh --backend=vllm` | High-performance, GPU servers |
| **LiteLLM** | `./local-first-setup.sh --backend=litellm` | Unified proxy, multiple backends |

📖 **Full guide:** [LOCAL_FIRST_SETUP.md](LOCAL_FIRST_SETUP.md)

</details>

> **Your data stays local. No cloud account, no telemetry, complete privacy.** 🔒
...
- **One of the following** (local options work without any account):
  - **Local:** Ollama, LM Studio, vLLM, or any OpenAI-compatible server
  - **Cloud:** NEAR AI account (free tier), OpenAI API key, or Anthropic API key
