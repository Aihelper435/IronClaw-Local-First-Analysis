...
/// Used for auto-detecting local LLM setup.
fn is_ollama_available() -> bool {
    use std::net::TcpStream;
    use std::time::Duration;

    // If OLLAMA_BASE_URL is explicitly set, assume user wants Ollama
    if std::env::var("OLLAMA_BASE_URL").is_ok() {
        return true;
    }

    // Quick TCP connect check to localhost:11434 (default Ollama port)
    // Timeout is 100ms to avoid blocking startup
