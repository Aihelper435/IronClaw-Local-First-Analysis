...
impl SetupWizard {
    ...
    async fn step_inference_provider(&mut self) -> Result<(), SetupError> {
        ...
        print_info("Select your inference provider (local options first for privacy):");
        println!();

        // Prioritize local options for privacy-conscious users
        let options = &[
            "Ollama           - local models, no API key needed [RECOMMENDED]",
            "OpenAI-compatible - local: vLLM, LiteLLM, LM Studio, etc.",
            "────────────────── Cloud Options ──────────────────",
            "NEAR AI          - multi-model access via NEAR account (free tier)",
        ...
        match choice {
            0 => self.setup_ollama()?,                          // Ollama (local)
            1 => self.setup_openai_compatible().await?,         // OpenAI-compatible (local)
            2 => {
                // Separator line - re-prompt
                print_info("Please select an actual provider option.");
                return Box::pin(self.step_inference_provider()).await;
            }
            3 => self.setup_nearai().await?,                    // NEAR AI (cloud)
            4 => self.setup_anthropic().await?,                 // Anthropic (cloud)
            5 => self.setup_openai().await?,                    // OpenAI (cloud)
