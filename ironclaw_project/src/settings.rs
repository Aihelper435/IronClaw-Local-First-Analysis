...
fn default_embeddings_provider() -> String {
    // Smart default: prefer local embeddings when Ollama is available
    if std::env::var("EMBEDDING_PROVIDER").is_ok() {
        // Explicit override - will be handled by config resolution
        return "nearai".to_string();
    }
    
    // Check if LLM_BACKEND is set to a local option
    ...
    // Check if OLLAMA_BASE_URL is set (implies local Ollama setup)
    if std::env::var("OLLAMA_BASE_URL").is_ok() {
        return "ollama".to_string();
    }
    
    // Default to nearai for cloud users
    "nearai".to_string()
}

fn default_embeddings_model() -> String {
    // Smart default: use appropriate model for the provider
    if let Ok(provider) = std::env::var("EMBEDDING_PROVIDER") {
        if provider == "ollama" {
            return "nomic-embed-text".to_string();
        }
    }
    
    // Check if local setup detected
    if let Ok(backend) = std::env::var("LLM_BACKEND") {
        if backend == "ollama" {
            return "nomic-embed-text".to_string();
        }
